  ...
  if (error instanceof Error) {
    ...
    if (error.message.includes('HTTP error')) {
      ...
      if (statusMatch) {
        ...
        switch (status) {
          case 400:
            return new ChatError('Invalid request. Please check your input.', 'BAD_REQUEST')
          case 401:
            return new ChatError('Authentication required. Please log in.', 'UNAUTHORIZED')
          case 403:
            return new ChatError('Access denied.', 'FORBIDDEN')
          case 404:
            return new ChatError('Service not found.', 'NOT_FOUND')
          case 429:
            return new ChatError('Too many requests. Please try again later.', 'RATE_LIMITED')
          case 500:
            return new ChatError('Server error. Please try again later.', 'SERVER_ERROR')
          default:
            return new ChatError(`Server error (${status}). Please try again.`, 'HTTP_ERROR')
        }
      }
    }
