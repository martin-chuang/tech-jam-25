...
import { validateFile, formatFileSize, getFileIcon } from '@/utils/file-validation'
...
export const useFileUpload = () => {
  ...
  const addFiles = useCallback(async (fileList: FileList) => {
    ...
    try {
      ...
      const existingFileSizes = files.map(f => f.size).reduce((sum, size) => sum + size, 0)
      const newFileSizes = filesToValidate.map(f => f.size).reduce((sum, size) => sum + size, 0)
  ...
  const getFileInfo = useCallback((file: UploadedFile) => ({
