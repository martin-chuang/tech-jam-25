...
class ChatStateHandlers:
    ...
    def processing_success_handler(
        ...
        if context:
            ...
            logger = logging.getLogger(__name__)
            session_id = context.get('session_id', 'unknown')
            logger.debug(f"LLM processing completed successfully for session {session_id}")
    ...
    def completion_success_handler(
        ...
        if context:
            ...
            logger = logging.getLogger(__name__)
            session_id = context.get('session_id', 'unknown')
            logger.info(f"Chat processing completed successfully for session {session_id}")
