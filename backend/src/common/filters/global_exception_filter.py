...
class GlobalExceptionFilter:
    ...
    def _create_error_response(
        self, status_code: int, message: str, error_type: str = "Error"
    ) -> Dict[str, Any]:
    ...
    def _handle_http_exception(self, error: HTTPException):
        ...
        response_data = self._create_error_response(
            status_code=error.code, message=error.description, error_type=error.name
        )
    ...
    def _handle_exception(self, error: Exception):
        ...
        response_data = self._create_error_response(
            status_code=500, message=message, error_type="InternalServerError"
        )
...
def register_custom_exception_handlers(app: Flask) -> None:
    ...
    def handle_validation_error(error):
        ...
        response_data = filter_instance._create_error_response(
            status_code=400, message=error.message, error_type="ValidationError"
        )
        if error.field:
    ...
    def handle_business_logic_error(error):
        filter_instance = GlobalExceptionFilter()
        response_data = filter_instance._create_error_response(
            status_code=422, message=error.message, error_type="BusinessLogicError"
        )
        if error.code:
    ...
    def handle_resource_not_found_error(error):
        filter_instance = GlobalExceptionFilter()
        response_data = filter_instance._create_error_response(
            status_code=404, message=error.message, error_type="ResourceNotFoundError"
        )
