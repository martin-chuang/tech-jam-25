...
class RetryUtils:
    ...
    @staticmethod
    def wait_with_backoff(attempt: int, base_delay: float = 1.0, max_delay: float = 60.0) -> None:
        """
        ...
        logger.info(f"Waiting {delay:.2f} seconds before retry (attempt {attempt + 1})...")
        time.sleep(delay)
    
    @staticmethod
    async def async_wait_with_backoff(attempt: int, base_delay: float = 1.0, max_delay: float = 60.0) -> None:
        """
