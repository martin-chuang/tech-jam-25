...
class FileConverter:
    ...
    def convert_to_markdown(self, file: FileStorage) -> str:
        ...
        try:
            ...
            elif content_type in ['application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'] or filename.endswith(('.doc', '.docx')):
            ...
            elif content_type in ['text/csv', 'application/json'] or filename.endswith(('.csv', '.json')):
    ...
    def _convert_docx_to_markdown(self, file: FileStorage) -> str:
        ...
        try:
            ...
            for paragraph in doc.paragraphs:
                if paragraph.text.strip():
                    ...
                    if paragraph.style.name.startswith('Heading'):
                        ...
                        try:
                            level_num = int(level)
                            markdown_content += f"{'#' * (level_num + 1)} {paragraph.text}\n\n"
                        except:
            ...
            self.logger.error(f"Error processing Word document {file.filename}: {str(e)}")
    ...
    def _convert_structured_to_markdown(self, file: FileStorage) -> str:
        ...
        try:
            ...
            self.logger.error(f"Error processing structured file {file.filename}: {str(e)}")
