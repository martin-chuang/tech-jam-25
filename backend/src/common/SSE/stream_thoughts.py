...
def process_chat_with_thoughts(chat_service, request_dto) -> Generator[str, None, None]:
    ...
    try:
        ...
        current_state, anonymised_prompt, anonymised_content = (
            chat_service._transition_to_anonymised(
                request_dto.prompt or "", request_dto.context or "", markdown_content
            )
        )
...
def _create_content_event(content: str) -> str:
    ...
    event_data = {"content": content, "timestamp": datetime.now().isoformat()}
    return f"data: {json.dumps(event_data)}\n\n"
